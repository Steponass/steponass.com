import Matter from 'matter-js'; // Add this import
...
registerBoundary(id, element, options = {}) {
  ...
  if (this.registeredBoundaries.has(id)) {
    this.log(`BoundaryMapper: Boundary "${id}" already registered, skipping`);
  ...
  try {
    ...
    if (physicsBody) {
      ...
      this.log(`BoundaryMapper: Registered boundary "${id}" at (${rect.left}, ${rect.top}), size: ${rect.width}x${rect.height}`);
